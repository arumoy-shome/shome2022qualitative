...
% We already have a lot of effort & work that has been put into
% identifying bias mitigation techniques at various "checkpoint" of the
% ML pipeline; but we lack work in studying the actual process of
% fairness testing! where is the bias located? we do not know, which
% bias mitigation techniques to use? we dont know...

% TODO get some news clipings of algorithmic fairness in medical,
% justice system, healthcare, etc. We need stories with compelling
% evidence for algorithmic bias & consequenly the need for testing for
% fairness in ML; use the baker2022algorithmic paper for this

% We need a holistic view of the entire system to identify root cause of
% bias; if we test for fairness only after training the model, it is
% very difficult to identify where exactly the bias was introduced. If
% we test at data level (DFM) and model level (MFM) then we have some
% "lead" identify root cause! So, if DFM is telling us there is bias,
% then it is an early indication that the data collection process has
% flaws; or alternatively, our initial design itself does not account
% for diversity which leads to a biased data collection process!
...
% algorithmic bias in a mature field of stufy & research: find papers
% from diverse backgrounds and summarise them here
% baker2022algorithmic: survey on algorithmic bias in education software
% link into fairness in ML & how it stems from algorithmic bias (use
% the wikipedia page for relevant discussion points)
% link to different fairness definitions & why we use group fairness;
% define it and list the relevant metrics here.
