...
%% substantially, making it difficult to detect and identify the fault.

In this paper, we conduct a priliminary exploratory study to evaluate
the effectiveness of fairness testing both before and after model
training. We propose a systematic methodology for ML fairness testing
...
bias in the training data (henceforth referred to as Data Fairness
...
(henceforth referred to as Model Fairness Metric or MFM). To evaluate
the effectiveness of our proposed testing approach, we analyse the
relationship between DFM and MFM empirically using five real-world
tabular datasets and four ML learning algorithms.

%% \begin{itemize}
%%   \item{\textbf{RQ1.}} \textbf{What is the relationship between DFM
%%     and MFM as the a) underlying data distribution b) the training
%%     sample size and c) the feature sample size changes?}    
%%   \item{\textbf{RQ1.}} \textbf{Can we use the relationship between DFM
%%     and MFM to locate where the bias lies within the ML pipeline?}
%% \end{itemize}

% perhaps better to say "... this is the first empirical study that
% tests for fairness both before and after training..."

% "...we are lso the first to apply root cause analysis..." this is
% wrong, we are not applying root cause analysis; we are proposing
% a technique to identify root cause perhaps? we also need to mention
% how this is beneficial to practitioners: it can help reduce
% development cycle time; which reduces costs; improves developer
% productivity blah blah blah...
...
%% I really like that DFM are so "cheap"; computationally there is
%% practically no overhead & it avoids a full training cycle!
...
negatively impact the fairness. We also find evidence of test
reduction when during the experimental design and evaluation phase of
ML models.
...
% How do we mitigate fairness?

% TODO review fairness related papers in IJCAI past preceedings; use
% the link luis shared for best papers

% TODO review AAAI past preceedings for fairness related papers as well

% fault localisation could also be included & relevant for our work
...
% definitions of fairness (in the context of ML and otherwise):
% castelnovo2022clarification
% hellman2020measuring
...
% fairness metrics & how it relates to oracles in software testing:
% zemel2013learning
% foulds2020intersectional
% speicher2018unified
% zhang2016identifying
...
% boxplot of the fairness metrics (DI top; SPD bottom)
% datasets along x axis; fairness metric value across y
...
% we need to expand here; what are the implications of this positive
% correlation?
...
%% next steps: can we use our work to identify root cause of bias?

% we have so many bias mitigation techniques; how do we know which one
% to use? endless experimentation required; with our work we hope to
% reduce this experimentation/debugging time; help practitioners
% identify the root cause & thus identify appropriate mitigation
% methods

% luis: if we address fairness at the data level then we can focus
% only on performance during model evaluation
...
%% different but relevant findings; then model complexity: some models
...
A trade-off between efficiency and performance is seen here. A
positive correlation between the DFM and MFM may indicate lack of
sufficient training data. Practitioners may consider collecting more
data is possible else opt for bias mitigations techniques that are
data-centric such as pre-processing techniques. With a negative
correlation, there is sufficient training data however there still may
be bias in the underlying ML model. Practitioners may opt to reduce
the training data and apply bias mitigation techniques for a more
efficient trainin cycle. No correlation between DFM and MFM gives
practitioners the option to go either way. They can reduce the
training data size at the cost of fairness or increase the training
data size at the cost of more compute.
...
%% maybe also comment on test reduction from the model angle, in our
%% study we observe that DTs always have a lower correlation compared
%% to other models; even with lesser data DTs are able to circumvent
%% bias in the data better than other models; this means that we
%% should prioritise testing with DT first? if it has fairness issues
%% other models are likely going to have worse issues in terms of
%% fairness?
