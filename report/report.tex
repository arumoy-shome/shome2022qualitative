...
%% TODO luis: feels a bit weak; any thoughts?
...
% DFM -> more biased data = higher DFM but the same does not hold true
% for MFM; after a certain "threshold" the models start to "learn" and
% make fairer decisions... data quality affects the metrics: simply
% adding more data does not always help; zhang21 showed that more data
% tends to make models unfair; we compared DFM and MFM at a given
% training sample size; they compared the models at different training
% sample size; different but relevant findings; then model complexity:
% some models are able to navigate
...
% We need a holistic view of the entire system to identify root cause
% of bias; if we test for fairness only after training the model, it
% is very difficult to identify where exactly the bias was
% introduced. If we test at data level (DFM) and model level (MFM)
% then we have some "lead" identify root cause! So, if DFM is telling
% us there is bias, then it is an early indication that the data
% collection process has flaws; or alternatively, our initial design
% itself does not account for diversity which leads to a biased data
% collection process!
...
% I think we should include model complexity and data quality as
% a side note; we want to show that our results are generalisable, so
...
% german dataset does not have that much bias but adult does? we won't
% have correlation between DFM & MFM when the bias in the data is low
% to begin with
...
%% plot of accuracy & fairness across all datasets, show that DTs are
%% inherently fair (perhaps cite some prior studies on this?), we can
%% reduce testing in the model land but first evaluating DTs? if DT is
%% unfair then chances are that other models will have worse biases?
