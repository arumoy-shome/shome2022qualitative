...
%% zhang2021ignorance 
%% - we use correlation (and linear regression) to analyse the
%% relationship between DFM & MFM
...
%% mention that these are all the datasets provided by the aif360 package
...
%% data preprocessing: we use the default pre-processing done by
%% aif360 library for the datasets; null values are dropped and
%% categorical variables are label encoded

%% we use scikit-learn for generating the training-testing splits and
%% the learning algorithms; we use standard scaler prior to training
