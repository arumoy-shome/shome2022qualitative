...
any of the stages can initiate a ``chain-reaction'' thoughout the
...
% TODO link between data and fairness testing is not clear
% add line on data-centric stuff
...
results are exploratory and open several intruiging avenues of
research.
...
\begin{description}
  ...
  \item[RQ2.] How does the training sample size affect the
    ...
    trade-off between fairness, efficiency and performance. In
    Section \ref{sec:implications} we provide some practical
    guidelines on how to best navigate this trade-off.
...
\begin{figure*}
  ...
  \caption{Machine Learning Lifecycle (adopted from \citeauthor{amershi2019software}).}
...
stages of the ML lifecycle adapted from prior scientific
literature \cite{amershi2019software,haakman2020ai,breck2019data}. The
stages can be classified into three distinct phases as depicted by the
different colors. Although the figure presents a linear flow amongst
the various phases of the lifecycle, there are often several feedback
loops that may influence one another. The experimental and production
phases are marked with a gray arrow in the top right corner as any
stage in these phases can loop back to any prior stage in the
lifecycle.
...
performance of the ML models has been purely based on functional
properties such as the correctness and model relevance. However, in
recent years, non-functional properties such as robustness, fairness,
security and explainability has also been
included \cite{zhang2020machine}.

% TODO the last sentence needs more work
...
systems---do not fail explicitely in case of an error. But continue to
operate, albeit with a degraded performance. Since the output of the
ML model may be utilised by other software sytems, the health of
...
% unclear what the benefit of doing this is...
% needs more refinement.
% shifting the DFM may speed up the experimental process
% and reduce costs by skipping a full train-test cycle

As seen in the figure, the state-of-the-art MFM are calculated using
the predictions of the model, after the model has been trained. In
contrast, we wish to evaluate the effectiveness of using DFM to
evaluate fairness of the ML model in the feature engineering phase,
prior to training. The objective of this study is to understand the
benefits and limitations of shifting
...
% TODO this entire section needs work
...
The interpretation of the correlation analysis varies based on the
control variable of the experiment and the span

Table \ref{tab:control} summarises the control variables in each
research question along with the span in which the correlation is
calculated.

A positive correlation indicates that the DFM and MFM
changed in the same direction as the control factor of the experiment
was varied. On the other hand, a negative correlation indicates that
the DFM and MFM changed in opposite direction as the control factor
was varied. A positive correlation means that the DFM and MFM convey
the same information, while a negative correlation indicates that they
do not convey the same information. What information they convey is
determined by the contol factor of the experiment and how it is
changed.
...
\begin{table}
  ...
  \begin{tabular}{l l r}
    \toprule
    \textbf{RQ} & \textbf{Control} & \textbf{Span}\\
    \midrule
    RQ1 & Data distribution & Within\\
    RQ2 & Training sample size & Within\\
    RQ3 & Training and feature sample size & Across\\
    \bottomrule
  \end{tabular}
  \caption{Control variables per research question}
  \label{tab:control}
\end{table}

\subsection{Analysis of Experimental Data}\label{sec:data-analysis}
...
study are represented using different colours---note that the model
``None'' represented in blue refers to the DFM. Both the fairness
metrics DI (top) and SPD (bottom) are presented in separate plots.
...
% TODO I still don't like the last sentence as much...
...
\begin{figure}
  \centering
  \includegraphics[width=0.95\linewidth]{shuffle.pdf}
  \caption{Rationale behind using smaller training sample to simulate
    change in the distribution of the training data.}
  \label{fig:shuffle}
\end{figure}
...
unfavourable ($Y=0$) examples. The large gray boxes represent the
original dataset while the blue boxes represent the training dataset.

The figures present three hypotherical iterations of the data sampling
...
the \emph{accuracy} and \emph{f1 score} of the models across the
training sample sizes. Next, we conduct \emph{student t-test} to
identify the smallest sample size where the performance of the models
is similar to that obtained when trained using the full training set.
...
iterations as explained in Section \ref{sec:results-full-rel}. This
can also be verified in Figure \ref{fig:lineplot--num-features--corr}
...
observed in the bottom right subplots which contain datasets with a
large training sample size but small feature sample size.
