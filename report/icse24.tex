...
% there was a comment for R1 regarding shuffling the our experimental data (which we do not do); perhaps clarify how spearkman correlation works here?

% ie. even though we do not suffle our experimental data, and it is sorted by the iteration number, this does not matter since spearman correlation will sort by the rank of the vector internally
...
% TODO start with a general motivation for this boxplot; we simply want to show the general trend prior to exploring the relationship between DFM & MFM!
...
metrics DI (top) and SPD (bottom) are presented in separate plots. Comparing the boxplots for DI and SDP, we note that they follow the same general pattern of distribution. Comparing the DFM and MFM for any given ML model and dataset, we observe a difference in the distribution and variability between the DFM and MFM. This is because, in addition to the randomness from the data shuffling in the training set, the models are assigned random initial states in every iteration thus resulting in different predictions across the iterations and consequently varying MFM values. Finally in several
...
% assigned random initial states in every iteration.
