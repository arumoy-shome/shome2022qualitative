...
% NOTE perhaps we can say that practitioners should start storing DFM
% and MFM from various training cycles for the ML systems in
% production. Analysis on the historical data reflects our 50
% iterations?
...
\begin{itemize}
  % personal opinions/attacks on style of writing/condescending
    % comments/no constructive feedback provided/running notes rather
  ...
  % comments about statements not mapping directly to citations
  ...
  \item ``We extend the above experiment further in two ways.'' -- The
    ...
    % NOTE single protected attribute because of prior literature and
    % we have to keep it similar since MFM is our baseline for
    % comparison
    % NOTE can also explain how the complexity rises when we consider
    % more than 1 protected attribute
  ...
  \item While training can be expensive for some modern ML tasks,
    ...
    % NOTE cite papers from google about training efficiency (luis
    % will know)
    % NOTE what about the impact in the environment: we wrote about
    % this in the discussion
    % NOTE regarding the i.i.d: entirety of ML is based on this
    % assumption; the reviewer has unrealistic expectations

    % \highlight{TODO}

  % misc/unsorted/no action required
