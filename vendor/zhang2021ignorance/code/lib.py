...
def get_adult():
    ...
    whole_datset_use, priviledgedclasslist = fit_string_data(
        whole_datset_use, headlist, protected_attribute, privileged_class_dic
    )

    dataset_orig = aif360.datasets.StandardDataset(
...
def get_PV_classic(classicclassifier, X, y):
    ...
    while cnt < len(noisedegreelist):
        ...
        for label in label_list:
            dic_label_newnum[
                label
            ] = 0  # initialize the dic; nothing is perturbed at the beginning
        ...
        for i in range(0, Y_changed.size):
            ...
            for label in label_list:
                ...
                if (
                    Y_changed[i] == label_list[cnnn]
                    and dic_label_newnum[label_list[cnnn]]
                    < float(dic_label_num[label_list[cnnn]]) * noisedegree
                ):
                    try:
    ...
    m, b = poly.polyfit(
        Xtest, Ytest, 1
    )  # conduct linear regression; b is the coefficient
    pv = 1 - abs(
        1 - abs(b)
    )  # mirror PV by one, so that PV increases up to 1, then worsen afterwards
    # pv = -b
...
def getdata_ricci():
    ...
    dataset_orig_bin = aif360.datasets.BinaryLabelDataset(
        favorable_label=1,
        unfavorable_label=0,
        df=proc_dataset,
        label_names=["Class"],
...
def getdata_credit():
    ...
    proc_dataset = dataset.replace(
        ["male", "female", "high school", "higher ed"], [1, 0, 1, 0]
    )
