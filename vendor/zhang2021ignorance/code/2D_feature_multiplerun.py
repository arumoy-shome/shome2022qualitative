...
def drawFig_fortestaccuracy(datasetname, protectedattribute, filepath):
    ...
    lines += ax1.plot(
        range_, metric_list, "--", color="black", label="test accuracy", linewidth=5
    )
    ax1.fill_between(
        range_,
        divia_add_metric_list,
    ...
    plt.savefig(
        "./plots/" + datasetname + "-" + protectedattribute + "-fn-testaccuracy.pdf",
        bbox_inches="tight",
    )
...
def drawFig(datasetname, protectedattribute, filepath):
    ...
    if "bank" or "german" in datasetname:
        plt.xticks(np.arange(min(range_), max(range_) + 1, 5))
    if "meps" in datasetname:
        plt.xticks(np.arange(min(range_), max(range_) + 1, 10))

    plt.savefig(
        "./plots/" + datasetname + "-" + protectedattribute + "-fn.pdf",
        bbox_inches="tight",
    )
